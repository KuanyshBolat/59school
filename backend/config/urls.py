...
def _internal_import_static(request):
    token_expected = os.environ.get('ADMIN_SETUP_TOKEN')
    token_given = request.GET.get('token') or request.headers.get('X-Admin-Token')
    token_given = _normalize_token_candidate(token_given)
    if not token_expected or token_given != token_expected:
    ...
    # Run the management command import_static_content
    try:
        base_dir = settings.BASE_DIR
        manage_py = os.path.join(base_dir, 'manage.py')
        subprocess.check_output(['python', manage_py, 'import_static_content'], stderr=subprocess.STDOUT)
...
if not settings.DEBUG:
    urlpatterns += [
        ...
        path('internal-import-static/', _internal_import_static),
    ]
