...
export default function Hero() {
  ...
  return (
    <section id="hero" className="relative pt-16 w-full h-[60vh] md:h-screen overflow-hidden">
      {/* Carousel */}
      <div className="relative w-full h-full">
        {visibleSlides.map((slide, index) => (
          ...
          >
            <img loading="lazy" src={slide.image || "/placeholder.svg"} alt={slide.title} className="w-full h-full object-cover" />
            <div className="absolute inset-0 bg-black/40" />
            <div className="absolute inset-0 flex flex-col items-center justify-center text-center px-4">
              <h1 className="text-3xl sm:text-4xl md:text-6xl font-bold text-white mb-4 text-balance">{slide.title}</h1>
              <p className="text-base sm:text-lg md:text-2xl text-white/90 max-w-2xl text-balance">{slide.subtitle}</p>
            </div>
          </div>
        ))}
      </div>
